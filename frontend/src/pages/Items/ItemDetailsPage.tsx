...
export default function ItemDetailsPage() {
  ...
  const initialMetadataText = useMemo(() => {
    return JSON.stringify(item?.metadata ?? {}, null, 2);
  }, [item]);

  const effectiveMetadataText =
    metadataText === "{}" ? initialMetadataText : metadataText;
  ...
  const onSave = () => {
    ...
    try {
      parsedMetadata = JSON.parse(effectiveMetadataText.trim() || "{}") as Record<
        string,
        unknown
      >;
    } catch {
